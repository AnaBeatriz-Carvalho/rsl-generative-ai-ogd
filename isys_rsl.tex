...
abrangendo bases internacionais e a Biblioteca Digital da SBC/SOL como fonte
adicional. [Resultados] A revisão incluiu \textbf{29 estudos} na síntese
(27 das bases internacionais e 2 da SBC/SOL).
\textbf{[ATUALIZAR APÓS EXTRAÇÃO: principais aplicações; tipos de dados;
...
guidelines and PRISMA~2020 reporting, covering international databases and the
SBC/SOL Digital Library as an additional source.
[Results] The review included \textbf{29 studies} in the synthesis
(27 from the international databases and 2 from SBC/SOL).
\textbf{[TO UPDATE AFTER EXTRACTION: main
...
especificamente --- ausência verificada por busca exploratória prévia nas mesmas
bases consultadas (Seção~\ref{subsec:busca}), restringindo os tipos de documento
a \textit{review}, \textit{survey} e \textit{systematic mapping} ---, lacuna que
justifica a presente RSL e orienta as questões formuladas na
...
\begin{description}
  \item[Validade de construto] Adequação das RQs e da \emph{string} de busca ao
        ...
        \emph{Mitigação:} string iterada por especialista, com calibração contra
        um conjunto de controle de artigos previamente reconhecidos como relevantes.
  ...
  \item[Validade externa] Generalização limitada pelo escopo de bases e idioma.
        Em particular, o recorte de idioma (inglês e português) pode subrepresentar
        produção em espanhol, com possível impacto sobre a cobertura do contexto
        latino-americano de governo aberto. \emph{Mitigação:} múltiplas bases e
        registro de limitações.
  \item[Cobertura da busca] A revisão apoiou-se exclusivamente na busca
        automática nas bases consultadas; não foi conduzido \emph{snowballing}
        para frente e para trás \citep{wohlin2014} sobre os estudos incluídos.
        Essa decisão pode ter deixado de capturar estudos relevantes não
        indexados nas bases consultadas ou conectados apenas por citação,
        especialmente literatura recente ou de comunidades regionais. A execução
        do \emph{snowballing} fica registrada como trabalho futuro.
...
\url{https://github.com/AnaBeatriz-Carvalho/democracia_legislativa_artigo}.
\end{materials}
