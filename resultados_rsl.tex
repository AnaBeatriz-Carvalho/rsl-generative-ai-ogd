...
\begin{tikzpicture}[
  ...
  ident/.style ={box, fill=blue!8,   text width=4.6cm},
  screen/.style={box, fill=green!8,  text width=4.6cm},
  elig/.style  ={box, fill=orange!10,text width=4.6cm},
  incl/.style  ={box, fill=red!8,    text width=5.2cm, font=\footnotesize\bfseries},
  excl/.style  ={box, fill=gray!10,  text width=3.6cm, font=\scriptsize},
  arr/.style={->, thick},
]
% ---------- Coluna A: Bases internacionais ----------
\node[ident] (idA) {\textbf{Identificação --- Bases internacionais}\\ Scopus 453; ACM 445;\\ IEEE 224; WoS 143\\ \textbf{Total: 1.265}};
\node[screen, below=of idA] (dedA) {Após deduplicação (Zotero)\\ \textbf{805 únicos} ($-$460 duplicatas)};
\node[screen, below=of dedA] (trA) {Triagem por título/resumo (Rayyan)\\ \textbf{46} buscados para recuperação};
\node[elig,   below=of trA]  (ftA) {Avaliados em elegibilidade\\ (texto completo): \textbf{27}};
\node[incl,   below=of ftA]  (inA) {Incluídos (bases internacionais): \textbf{27}};

% ---------- Coluna B: SBC/SOL ----------
\node[ident,  right=2.2cm of idA] (idB) {\textbf{Identificação --- SBC/SOL}\\ Busca booleana integrada\\ (fonte adicional, sem export.\\ padronizada)\\ \textbf{Total: 23}};
\node[screen, below=of idB] (dedB) {Após deduplicação manual\\ \textbf{23 únicos} (sem duplicatas)};
\node[screen, below=of dedB] (trB) {Triagem por título (manual)\\ \textbf{9} buscados para recuperação};
\node[elig,   below=of trB] (ftB) {Avaliados em elegibilidade\\ (texto completo): \textbf{9}};
\node[incl,   below=of ftB] (inB) {Incluídos (SBC/SOL): \textbf{2}};

% ---------- Caixas de exclusão (laterais) ----------
\node[excl, right=0.35cm of trA] (exA1) {Excluídos t/r:\\ \textbf{759}};
\node[excl, right=0.35cm of ftA] (exA2) {Não recuperados: \textbf{19}\\ Excluídos c/ motivo: \textbf{0}};

\node[excl, left=0.35cm of trB] (exB1) {Excluídos por\\ título: \textbf{14}};
\node[excl, left=0.35cm of ftB] (exB2) {Excluídos c/ motivo: \textbf{7}\\ {\scriptsize CE3+CE2+CE1 (1)}\\ {\scriptsize CE1 (5) · CI5 (1)}};

% ---------- Caixa final (corpus) ----------
\node[incl, below=1.0cm of $(inA)!0.5!(inB)$, text width=6.6cm, fill=red!14] (total)
  {\textbf{Corpus final de síntese: 29 estudos}\\ (27 bases internacionais + 2 SBC/SOL)};
...
% Setas para caixas laterais
\draw[arr] (trA.east) -- (exA1.west);
\draw[arr] (ftA.east) -- (exA2.west);
\draw[arr] (trB.west) -- (exB1.east);
\draw[arr] (ftB.west) -- (exB2.east);

% Convergência ao corpus final
\draw[arr] (inA.south) -- ($(inA.south)+(0,-0.35)$) -| (total.north);
\draw[arr] (inB.south) -- ($(inB.south)+(0,-0.35)$) -| (total.north);
\end{tikzpicture}
