...
Delimita-se, desde já, o alcance do que será demonstrado. O termo \textit{democratização do acesso}, presente no título e na motivação deste trabalho, designa um horizonte de impacto social cuja realização plena depende de fatores que extrapolam o escopo de uma dissertação. O que esta pesquisa efetivamente projeta e avalia são \textbf{pré-condições técnicas} dessa democratização: a estruturação confiável dos dados, a qualidade factual e a rastreabilidade das respostas e a disponibilização de interfaces de baixa barreira de entrada. A avaliação de acessibilidade e de impacto junto a cidadãos reais, que exigiria um estudo com participantes humanos, é reconhecida como trabalho futuro e não constitui objeto da avaliação aqui proposta. Cabe ainda distinguir, desde a introdução, as duas barreiras que o artefato busca reduzir e que serão tratadas de forma articulada ao longo do texto: a \textbf{barreira técnica} --- a necessidade de manejar \textit{endpoints}, formatos XML/JSON e linguagens de consulta para obter o dado bruto --- e a \textbf{barreira cognitiva} --- a dificuldade de interpretar dados e visualizações sem letramento estatístico ou familiaridade com o jargão legislativo.
...
\begin{citacao}
\textbf{Hipótese:} A combinação do processamento dos dados abertos, classificação temática com o Mistral 7B Instruct e interface visual/conversacional é capaz de estruturar e tornar consultáveis dados parlamentares abertos. Especificamente, espera-se que: (a) a classificação temática automática dos discursos apresente acurácia superior à de um classificador por palavras-chave utilizado como linha de base; (b) as respostas do agente conversacional sejam majoritariamente coerentes com os dados recuperados, com a taxa de coerência factual reportada e comparada a uma configuração sem recuperação contextual; e (c) as respostas mantenham rastreabilidade explícita --- podendo ser associadas aos registros de origem ---, com essa rastreabilidade caracterizada tanto quantitativa quanto qualitativamente. Em vez de fixar limiares absolutos de aprovação ou reprovação, a pesquisa adota uma postura orientada à caracterização: as taxas efetivamente alcançadas serão reportadas, discutidas à luz dos \textit{baselines} adotados e das limitações do corpus de avaliação, e os casos de insucesso --- quando ocorrerem --- serão analisados como resultado científico central da dissertação.
\end{citacao}
