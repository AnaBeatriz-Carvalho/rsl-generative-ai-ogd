...
Lee et al. \cite{Lee2017}, através do desenvolvimento do VLAT (\textit{Visualization Literacy Assessment Test}), demonstraram que nem todos os usuários conseguem interpretar visualizações gráficas complexas. Especificamente, o estudo aponta que:

\begin{itemize}
\item Usuários com pouca exposição a gráficos estatísticos frequentemente cometem erros na leitura de eixos, escalas e representações multidimensionais.
\item Visualizações sofisticadas (heatmaps, scatter plots 3D) podem confundir mais do que esclarecer para usuários sem expertise.
\item A compreensão visual de dados é uma competência que varia significativamente entre indivíduos.
\end{itemize}

\subsection{Barreiras Cognitivas para Usuários Não-Especialistas}

Hornung et al. \cite{Hornung2015} apontam que usuários sem formação em estatística ou ciência de dados frequentemente enfrentam barreiras cognitivas significativas ao tentar analisar dados complexos. Especificamente:

\begin{itemize}
\item Falta de familiaridade com conceitos como mediana, distribuição, correlação etc.
\item Dificuldade em construir hipóteses e validá-las contra dados.
\item Sobrecarga cognitiva ao lidar simultaneamente com múltiplas dimensões de informação.
\end{itemize}
