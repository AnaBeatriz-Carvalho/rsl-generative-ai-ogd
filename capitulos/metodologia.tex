...
A fase de avaliação foi conduzida sem entrevistas, questionários, observação comportamental ou qualquer coleta de dados com participantes humanos. A avaliação incidiu sobre o artefato computacional, os dados públicos processados e as respostas geradas pelo modelo. Dessa forma, a pesquisa utiliza exclusivamente dados abertos, informações de domínio público e análises técnicas do sistema.
...
\subsubsection{Método}
Foi elaborado um conjunto de 40 perguntas representativas de consulta, definido \textit{a priori} e estratificado pelas classes de pergunta sobre os discursos do corpus avaliado: factuais diretas, de agregação/contagem, de comparação, de busca por tema e perguntas sem resposta no corpus --- estas últimas para testar o reconhecimento da ausência de dados. A avaliação recaiu sobre os discursos do \textit{snapshot} congelado descrito no Capítulo~\ref{cap:resultados}; votações não integraram esta frente. Para cada pergunta, definiu-se \textit{a priori} a evidência esperada (\textit{gold}) a partir dos dados oficiais coletados da API do Senado Federal. Diferentemente do protocolo originalmente proposto --- que previa a classificação das respostas em quatro categorias (correta, parcialmente correta, incorreta e não respondida) e uma ablação entre as configurações com e sem recuperação contextual ---, a avaliação efetivamente conduzida comparou \textbf{quatro modelos geradores} sob um mesmo \textit{pipeline} de recuperação fixo, mantendo constantes o corpus, os \textit{prompts} e os hiperparâmetros, de modo a atribuir ao modelo qualquer diferença observada. Cada resposta foi avaliada por uma escala de \textbf{suporte semântico} (1--4), que afere se a evidência citada de fato sustenta a afirmação, complementada por completude (1--5), adequação ao português formal (1--5) e reconhecimento correto de ausência (recusa, 0/1). A anotação foi conduzida por \textbf{anotador único}, em condição cega quanto à identidade do modelo (ordem das respostas aleatorizada e modelo ocultado), com a evidência-fonte do corpus disponibilizada ao lado de cada resposta; por se tratar de avaliador único, não se calculou concordância interanotador (Kappa de Cohen), o que se declara como limitação entre as ameaças à validade interna. O protocolo de anotação foi fixado antes da execução. Constituem desvios assumidos em relação ao protocolo proposto: a substituição do esquema de quatro categorias pela escala de suporte semântico, a ausência da ablação com/sem recuperação --- substituída pela comparação entre modelos --- e a anotação por avaliador único em vez de dupla anotação com Kappa.

\subsubsection{Métricas}
\begin{itemize}
    \item \textbf{Suporte semântico (1--4):} grau em que a evidência citada sustenta a afirmação --- métrica primária de qualidade da resposta.
    \item \textbf{Completude (1--5):} grau em que a resposta contempla o que foi perguntado.
    \item \textbf{Adequação ao português formal (1--5).}
    \item \textbf{Recusa correta (0/1):} reconhecimento da ausência de dados nas perguntas sem resposta no corpus.
    \item Tipos de erro observados, como atribuição a parlamentar incorreto, omissão, generalização indevida ou citação que não sustenta a afirmação.
\end{itemize}
...
Diz respeito a fatores que podem distorcer a relação entre o artefato e os resultados observados. O principal risco é o viés de anotação --- critérios subjetivos na construção dos gabaritos e eventual não-independência entre anotadores. Na avaliação de geração efetivamente conduzida, a anotação foi realizada por \textbf{anotador único}, em condição cega quanto à identidade do modelo (ordem aleatorizada e modelo ocultado), com critérios de pontuação definidos \textit{a priori} e a evidência-fonte do corpus disponibilizada ao lado de cada resposta. Por se tratar de avaliador único, \textbf{não foi calculada a concordância interanotador (Kappa de Cohen)}, o que constitui limitação assumida: as médias por modelo refletem o julgamento de um único avaliador e podem conter viés sistemático não detectável por concordância; uma mitigação parcial possível seria um teste-reteste intra-anotador sobre uma subamostra. Para a frente de classificação temática, ainda não executada, permanece previsto o protocolo de dupla anotação independente com limiar de Kappa pré-fixado e terceiro anotador para desempate.

\subsection{Validade Externa}
Refere-se à generalização dos resultados. Embora o artefato processe o recorte de 2024 a 2026, a avaliação empírica recaiu sobre um \textit{snapshot} congelado de 152 discursos do Senado Federal, coletados entre 5 e 16 de maio de 2025, de modo que os achados podem não se transferir para o recorte completo, para outros períodos, outras casas legislativas ou idiomas; do mesmo modo, ainda que a comparação abranja quatro modelos, conclusões sobre desempenho não se estendem automaticamente a outros modelos ou configurações. Essas fronteiras são declaradas, e a arquitetura modular do artefato é apontada como caminho para replicação em outros contextos.
