...
\begin{itemize}
    ...
    \item \textbf{Carregamento:} Armazenamento em banco de dados local (SQLite) para consulta rápida durante operações de Chatbot e Dashboard.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Motor de Resposta (RAG Simplificado):} O módulo recebe perguntas do usuário em linguagem natural via Chatbot, recupera dados relevantes do banco de dados estruturado (usando busca estruturada por palavras-chave sobre a base local), e instrui o Mistral 7B a gerar respostas contextualizadas, didáticas e alinhadas aos dados reais.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Inicialização:} Aplicação \textit{Streamlit} carrega dados pré-processados do banco de dados local.
    ...
    \item \textbf{Resposta:} Mistral 7B gera resposta em linguagem natural, citando quais dados foram utilizados.

    \item \textbf{Exibição:} Resposta é exibida no Chatbot. Usuário pode fazer pergunta de acompanhamento.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{2º Trimestre 2026 (Jul-Set):} Execução dos testes técnicos de acurácia, avaliação factual das respostas do agente conversacional, verificação de rastreabilidade e execução de cenários representativos de consulta. Início paralelo da escrita de artigo científico visando a conferência PROPOR 2026.
