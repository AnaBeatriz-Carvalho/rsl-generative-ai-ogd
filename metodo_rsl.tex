...
\rev{A consulta à SBC/SOL foi realizada manualmente pela interface de busca da própria plataforma --- baseada em \textit{Open Journal/Conference Systems}, com suporte booleano limitado e sem exportação padronizada (BibTeX/RIS) ---, e não por \textit{script} ou API. Empregou-se uma \textit{string} equivalente em português, com os termos de IA Generativa e LLMs combinados aos de dados governamentais, governo digital e transparência pública. Os registros retornados foram triados e deduplicados manualmente, aplicando-se o mesmo recorte temporal (a partir de 2019) e o mesmo critério de idioma (CI4) das demais bases. Esse tratamento manual, imposto pela ausência de exportação padronizada, é o motivo de a SBC/SOL ser reportada em fluxo separado no diagrama PRISMA (Seção~\mbox{\ref{sec:resultados}}).}
...
\rev{O tipo de veículo --- inclusive periódicos de iniciação científica ou anais sem indicação clara de revisão por pares de mérito --- foi registrado como metadado bibliográfico, mas não é tomado como proxy do rigor do estudo. A avaliação de qualidade foi aplicada item a item, de forma uniforme, a todos os estudos, independentemente do veículo: o critério CI2 (estudo primário revisado por pares) admite esses veículos no corpus, e eventuais diferenças de qualidade emergem do escore item a item, e não de uma penalização atribuída ao veículo. De fato, o único estudo do corpus oriundo de um veículo de iniciação científica alcançou escore alto na avaliação, por mérito próprio, o que reforça que o rigor editorial do veículo e o rigor metodológico do estudo são dimensões distintas.}
